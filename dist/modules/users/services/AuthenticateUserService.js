...
let AuthenticateUserService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if (!user) throw new _AppError.default('Incorrect e-mail/password combination', 401);
    const passwordMatched = await this.hashProvider.compareHash(password, user.password);
    if (!passwordMatched) throw new _AppError.default('Incorrect e-mail/password combination', 401);
    const {
      secret,
      expiresIn
    } = _auth.default.jwt;
    ...
    if (!secret) {
      throw new _AppError.default('APP_SECRET not found.', 400);
    }

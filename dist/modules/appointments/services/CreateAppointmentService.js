...
let CreateAppointmentService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async execute({
    ...
    if ((0, _dateFns.isBefore)(appointmentDate, Date.now())) {
      throw new _AppError.default('You can not create an appointment on past date.');
    }

    if (user_id === provider_id) {
      throw new _AppError.default('You can not create an appointment with yourself.');
    }

    if ((0, _dateFns.getHours)(appointmentDate) < 8 || (0, _dateFns.getHours)(appointmentDate) > 17) {
      throw new _AppError.default('You can only create appointments between 8:00am and 5:00pm');
    }
    ...
    const dateFormatted = (0, _dateFns.format)(appointmentDate, "dd/MM/yyyy '-' HH:mm'h'");
